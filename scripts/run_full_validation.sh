...
python -m solomuse_data.cli canonicalize --config $CONFIG --dataset $DATASET

echo ""
echo "[2/8] Build Backing/Solo Pairs"
python -m solomuse_data.cli build-pairs --config $CONFIG --dataset $DATASET

echo ""
echo "[3/8] Segment Audio"
python -m solomuse_data.cli segment --config $CONFIG --dataset $DATASET

echo ""
echo "[4/8] Extract Situation Layer (Limit: $LIMIT)"
python -m solomuse_data.cli situation --config $CONFIG --dataset $DATASET $LIMIT_FLAG --overwrite

echo ""
echo "[5/8] Build Intent Targets (Limit: $LIMIT)"
python -m solomuse_data.cli intent-targets --config $CONFIG --dataset $DATASET $LIMIT_FLAG --overwrite

echo ""
echo "[6/8] Build Renderer Targets (Limit: $LIMIT)"
python -m solomuse_data.cli renderer-targets --config $CONFIG --dataset $DATASET $LIMIT_FLAG --overwrite

echo ""
echo "[7/8] Train Intent Planner"
echo "(Note: if no validation split, will fallback to train loss)"
python -m solomuse_data.cli train-intent --config $CONFIG --dataset $DATASET

echo ""
echo "[8/8] Train Renderer"
python -m solomuse_data.cli train-renderer --config $CONFIG --dataset $DATASET
...
python -m solomuse_data.cli infer-intent --config $CONFIG --dataset $DATASET --segment-dir "$SEGMENT_DIR"

echo "  -> Running Renderer Inference (End-to-End audio)"
python -m solomuse_data.cli render-segment --config $CONFIG --dataset $DATASET --segment-dir "$SEGMENT_DIR"
